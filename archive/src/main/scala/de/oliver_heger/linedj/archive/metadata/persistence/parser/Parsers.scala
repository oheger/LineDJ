...
/**
  * A trait defining the API of a parser combinator library.
  *
  * This code is a slightly modified version from the ''Parsers'' trait from
...
case class Location(input: String, offset: Int = 0) {
  ...
  def currentLine: String =
    if (input.length > 1) {
      val it = input.lines.drop(line-1)
