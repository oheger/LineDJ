...
object ServerRunner:
  ...
  /**
    * A prefix used for log messages. This makes sure that the server name is
    * prepended to all log output generated by the runner.
    */
...
class ServerRunner(locatorFactory: ServerLocator.LocatorFactory = ServerLocator.newLocator)
  ...
  /**
    ...
    *
    * @param serverName a name for the server; this is mainly used for log
    *                   output, but also to generate actor names
    * @param controller the controller for the server
    * @return a [[ServerHandle]] for this server
    */
  def launch(serverName: String, controller: ServerController): ServerHandle =
    log.info(LogPrefix + "Launching server controlled by {}.", serverName, controller.getClass.getSimpleName)
    ...
    val startFuture = (for
      ...
      _ <- startLocator(serverName, serverParams.optLocatorParams, factory)
    yield startup) andThen :
      case Success(startupData) =>
        log.info(
          LogPrefix + "HTTP server is listening on port {}.",
          serverName,
          startupData.binding.localAddress.getPort
        )
      case Failure(exception) =>
        log.error(LogPrefix + "Failed to start HTTP server.", serverName, exception)
    ...
    val gracefulShutdownFuture = enableGracefulShutdown(startFuture, shutdownPromise.future) map : context =>
      log.info(LogPrefix + "HTTP server has shut down.", serverName)
    ...
    new ServerHandle:
      ...
      override def shutdown(): Unit =
        if shutdownPromise.trySuccess(Done) then
          log.info(LogPrefix + "HTTP server was shut down using its handle.", serverName)
  ...
  /**
    ...
    */
  private def startLocator(serverName: String,
                           optParams: Option[ServerLocator.LocatorParams],
                           actorFactory: ManagingActorFactory): Future[Done] =
    optParams.foreach: params =>
      log.info(LogPrefix + "Starting locator for server.", serverName)
      locatorFactory(s"${serverName}_locator", params)(using actorFactory)
