...
object ArchiveStateMonitor:
  ...
  /**
    * Type alias for a function this actor uses to generate the HTTP request to
    * send to the archive. The function expects the current data managed by 
    * this actor (which may not yet be available) and can derive the request
    * from there.
    *
    * @tparam DATA the type of the data to be managed
    */
  type RequestFunc[DATA] = Option[DATA] => HttpRequest
      ...
      Behavior[ArchiveMonitorCommand[DATA, STATE]] =
        Behaviors.receiveMessage:
          ...
          case ArchiveCheckCommand.TriggerCheck(replyTo) =>
            ...
            val request = params.requestFunc(optData.map(_._1))
            val futEval = for
              result <- HttpRequestSender.sendRequestSuccess(
