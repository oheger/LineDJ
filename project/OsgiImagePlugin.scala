...
object OsgiImagePlugin extends AutoPlugin {
  ...
  /**
    * Creates a dynamic task to obtain the artifacts generated by the given
    * task for the projects the current project depends on. Using a dynamic
    * task is needed; otherwise the results of the task from referenced
    * projects cannot be obtained.
    *
    ...
    */
  private def projectArtifactsForTask(task: TaskKey[File]): Def.Initialize[Task[Seq[Option[sbt.File]]]] = Def.taskDyn {
