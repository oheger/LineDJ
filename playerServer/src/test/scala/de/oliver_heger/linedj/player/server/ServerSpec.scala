...
class ServerSpec(testSystem: ActorSystem) extends TestKit(testSystem) with AnyFlatSpecLike with BeforeAndAfterAll
  ...
  "Server" should "run the server" in :
    val factory = mock[ServiceFactory]
    val runner = mock[ServerRunner]
    val handle = mock[ServerRunner.ServerHandle]
    doReturn(handle).when(runner).launch(eqArgs(Server.ServerName), any[Controller]())
    doReturn(runner).when(factory).createServerRunner()
    val server = new Server(factory)
