...
/**
  ...
  */
class RelayActorSpec(testSystem: ActorSystem) extends TestKit(testSystem) with ImplicitSender with AnyFlatSpecLike
  ...
  private class RemoteRelayActorTestHelper:
    ...
    def provideRemoteActors(): ActorRef =
      pathActorMapping foreach { e =>
        relayActor ! LookupActor.RemoteActorAvailable(e._1, e
          ._2._2)
      }
