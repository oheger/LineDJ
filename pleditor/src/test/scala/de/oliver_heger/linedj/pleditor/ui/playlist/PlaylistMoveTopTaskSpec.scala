...
/**
  * Test class for ''PlaylistMoveTopTask''.
  */
class PlaylistMoveTopTaskSpec extends AnyFlatSpec with Matchers with MockitoSugar {
  /**
    * Creates a test task with a mock controller.
    * @return the test task
    */
  private def createTask(): PlaylistMoveTopTask = {
    new PlaylistMoveTopTask(mock[PlaylistController])
  }

  "A PlaylistMoveTopTask" should "not be enabled if there is no selection" in {
  ...
  it should "move selected elements to the top" in {
    val context = mock[PlaylistSelectionContext]
    ...
    model should be(java.util.Arrays.asList(2, 4, 5, 8, 0, 1, 3, 6, 7, 9, 10))
    verify(tabHandler).tableDataChanged()
    verify(tabHandler).setSelectedIndices(Array(0, 1, 2, 3))
  }
}
