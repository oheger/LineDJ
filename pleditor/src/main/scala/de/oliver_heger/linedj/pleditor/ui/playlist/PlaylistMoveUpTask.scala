...
/**
  ...
  * playlist up by one position.
  *
  * @param controller the ''PlaylistController''
  */
class PlaylistMoveUpTask(controller: PlaylistController)
  ...
  override def updatePlaylist(context: PlaylistSelectionContext): Unit =
    ...
    for idx <- orderIndices do
      ...
      model.set(idx, t)
    context.tableHandler.rowsUpdated(context.minimumSelectionIndex - 1,
      context.maximumSelectionIndex)
  ...
  /**
    ...
    *             is a selection, and the first element is not selected.
    */
