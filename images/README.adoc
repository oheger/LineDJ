...
The LineDJ applications run on a Java environment using any recent Java version. (Typically, no specific Java features or APIs are used, but the JDK requirements are enforced by the version of Scala in use.) As UI
technology, the applications use https://openjfx.io/[JavaFX].

From Java 11 on, JavaFX is no longer part of the core environment. This complicates
...
One way to achieve this is to create a
...
Alternatively, it is possible to use a standard JDK or JRE and pass the additional required JavaFX modules as parameters. The modules are available in the JavaFX SDK that can be downloaded in the desired version from the https://gluonhq.com/products/javafx/[JavaFX downloads page]. Unzip the SDK to a folder. Assuming that the variable `JAVA_FX_SDK` points to this folder and the `JAVA_HOME` variable is set correctly, the following command starts a LineDJ OSGi application:
