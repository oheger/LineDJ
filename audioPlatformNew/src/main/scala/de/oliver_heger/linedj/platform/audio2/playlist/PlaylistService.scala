...
/**
  * Interface for a service that allows basic operations on playlists.
  *
  ...
  * @tparam SongID   the type for the IDs to reference songs
  */
trait PlaylistService[Playlist, SongID]:
  /**
    * Type definition for a list of songs. A _Playlist_ may consist of one or
    * multiple lists of this type. The difference between a _SongList_ and a
    * _Playlist_ is that the former is only a sequence of songs while the
    * latter also defines a position, i.e. the current song to be played.
    */
  type SongList = List[SongID]

  /**
    * Returns the size of the specified _Playlist_ This is the number of
    * songs contained in this list.
    *
    * @param playlist the _Playlist_
  ...
  /**
    ...
    * already been played, result is _None_.
    *
    * @param playlist the _Playlist_
    * @return an [[Option]] for the current song in the playlist
    */
  def currentSong(playlist: Playlist): Option[SongID]

  /**
    * Returns the (0-based) index of the current song in the specified
    * playlist. If the playlist has no current song, result is _None_.
    *
    * @param playlist the _Playlist_
  ...
  /**
    * Returns a new _Playlist_ instance whose current position is moved to
    ...
    * If the current song was the last song, result is _None_.
    *
    * @param playlist the _Playlist_
  ...
  /**
    * Transforms the given _Playlist_ into a list of songs that contains all
    * songs in order, independent of the current position of the playlist.
  ...
  /**
    * Generates a _Playlist_ from a sequence of songs and the index of the
    * current song. If the index is invalid (less than zero or greater or equal
    * than the size of the playlist), result is _None_.
    *
    * @param songs        the sequence of songs
    * @param currentIndex the index of the current song (0-based)
    * @return an [[Option]] with the resulting _Playlist_
    */
  def toPlaylist(songs: SongList, currentIndex: Int): Option[Playlist]

  /**
    * Returns a _Playlist_ with an updated current song. This function
    * creates a _Playlist_ with the same songs as the original one, but the
    ...
    * playlist), result is _None_.
    *
    * @param playlist     the original _Playlist_
