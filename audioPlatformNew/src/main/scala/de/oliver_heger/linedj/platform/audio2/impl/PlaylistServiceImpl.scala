...
/**
  * An object with a concrete implementation of [[PlaylistService]] that is
  * based on [[Playlist]] objects.
  */
object PlaylistServiceImpl extends PlaylistService[Playlist, String]:
  override def size(playlist: Playlist): Int =
    playlist.pendingSongs.size + playlist.playedSongs.size

  override def currentSong(playlist: Playlist): Option[String] =
  ...
  @tailrec override def incrementPlaylistSeqNo(seqNo: Int): Int =
    val next = seqNo + 1
    if next != PlaylistService.SeqNoInitial then next
