...
object ID3FrameExtractor {
  ...
  private val TagsV3 = Vector("TIT2", "TPE1", "TALB", "TYER", "TRCK")
  ...
  private val DirectFrameDataHandler = new FrameDataHandler {
    /**
      ...
      */
    override def frameData(nextChunk: ByteString): ByteString = nextChunk
  ...
  /**
    ...
    * ''VersionData'' object.
    *
    * @param data         the content of the frame as byte string
  ...
  private case class VersionData(nameLength: Int, sizeLength: Int,
    ...
    /**
      ...
      *
      * @param header a buffer with the bytes of the tag header
      * @param ofs    the offset into the array
      * @return the tag name as string
      */
    def extractTagName(header: ByteString, ofs: Int): String =
      ID3Tag.extractName(header, ofs, nameLength)

    /**
      * Extracts the size of the tag's content from the given header buffer.
      * This is the size without the header.
      *
  ...
  private trait FrameDataHandler {
    /**
      ...
      *         processing
      */
    def canProcess(nextChunk: ByteString): Boolean
