...
object Server:
  ...
  final val ServerName = "archiveServer"

  private val log = LogManager.getLogger(classOf[Server])

  /**
    * The main function of the archive server. Creates the central actor system
...
/**
  * The main class for the archive server. The class launches a server
  * implementation that loads the content of a local media archive and provides
  * access to the single media, artists, albums, and songs via a REST API.
  *
