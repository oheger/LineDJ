...
/**
  * Test class for [[MediaFileResolverLocal]].
  */
class MediaFileResolverLocalSpec(testSystem: ActorSystem) extends TestKit(testSystem) with AsyncFlatSpecLike
  with BeforeAndAfterAll with Matchers with MediaFileTestSupport:
  def this() = this(ActorSystem("MediaFileResolverLocalSpec"))
  ...
  "localFileResolverFunc" should "return a successful Future for a resolvable file" in :
    ...
    val resolverFunc = MediaFileResolverLocal.localFileResolverFunc(List(archive2Config, archive1Config))
    for
  ...
  it should "return a correct failed Future for a file that cannot be resolved" in :
    ...
    val resolverFunc = MediaFileResolverLocal.localFileResolverFunc(List(archiveConfig))
  ...
  it should "return a correct failed future for a file if the archive cannot be resolved" in :
    ...
    val resolverFunc = MediaFileResolverLocal.localFileResolverFunc(List.empty)
    recoverToExceptionIf[MediaFileResolver.UnresolvableFileException]:
      resolverFunc(fileID, downloadInfo)
    .map: exception =>
      exception.fileID should be(fileID)
