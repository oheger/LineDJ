...
object ArchiveContentActor:
  ...
  /**
    ...
    * instances.
    */
  final val behavior: Factory = () => handleArchiveCommand(Nil)

  /**
    * The main command handler function for the archive content actor.
    *
    * @param media the current list of available media
    * @return the updated behavior
    */
  private def handleArchiveCommand(media: List[ArchiveModel.MediumOverview]): Behavior[ArchiveContentCommand] =
    Behaviors.receive:
      case (ctx, ArchiveContentCommand.AddMedium(medium)) =>
        ctx.log.info("Added medium {}.", medium.overview)
        handleArchiveCommand(medium.overview :: media)

      case (_, ArchiveContentCommand.GetMedia(replyTo)) =>
        replyTo ! GetMediaResponse(media)
        Behaviors.same
