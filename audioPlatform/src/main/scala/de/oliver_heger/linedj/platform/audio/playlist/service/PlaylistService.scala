...
object PlaylistService extends
  ...
  override def playlistEquals(playlist1: Playlist, playlist2: Playlist): Boolean = {
    ...
    (Boolean, SongList) =
      if (idx < 0) compareSubLists(lst1, lst2.tail, idx + 1, lst2.head :: delta)
      else if (idx > 0) compareSubLists(lst1.tail, lst2, idx - 1, lst1.head :: delta)
      else lst1 match {
    ...
    if (pending1Cnt + played1Cnt != pending2Cnt + played2Cnt) false
    else if (pending1Cnt == pending2Cnt) playlist1 == playlist2
    else {
