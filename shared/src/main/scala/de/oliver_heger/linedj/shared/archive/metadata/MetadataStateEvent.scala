...
/**
  * A trait defining a hierarchy of events related to the state of the 
  * metadata management (aka the media archive).
  *
  * The metadata manager actor generates a number of events when there are
  * important changes in the state of the archive, e.g. when a new scan
  * operation starts, a scan operation completes, the metadata of a medium
  ...
  *
  * All events generated by the metadata manager are subclasses of this trait.
  */
sealed trait MetadataStateEvent extends RemoteSerializable

/**
  * A metadata state event indicating the start of a media scan operation.
  *
...
/**
  ...
  * component that is going to add metadata to the archive typically triggers
...
/**
  ...
  * media. Registered state listeners are passed this event whenever all 
  * metadata for a medium has been extracted. After receiving this message, the
  * metadata can be requested in a second step. This is more efficient than
  * receiving chunks for updates.
  *
  * During a single metadata extraction operation that is not canceled a
...
/**
  ...
  *
  * When a metadata state listener registers at the archive it automatically
...
/**
  * A metadata state event propagating statistics about the media archive to
