...
package de.oliver_heger.linedj.test
...
object RecordingSchedulerSupport:
   ...
   */
  def expectInvocation(queue: java.util.concurrent.BlockingQueue[SchedulerInvocation],
                       timeout: FiniteDuration = DefaultQueueTimeout): SchedulerInvocation =
...
trait RecordingSchedulerSupport extends SchedulerSupport:
  import RecordingSchedulerSupport.*
   ...
   *             queue
   */
  override def scheduleMessage(initialDelay: FiniteDuration,
  ...
  /**
    ...
    */
  override def scheduleMessageOnce(delay: FiniteDuration, receiver: ActorRef, message: Any): Cancellable =
    val cancellable = new CancellableImpl
