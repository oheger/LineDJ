...
import akka.actor.{Actor, ActorLogging, ActorRef, OneForOneStrategy, Props, SupervisorStrategy, Terminated, typed}
...
class RadioDataSourceActor(config: PlayerConfig, eventManager: typed.ActorRef[RadioEvent])
  ...
  private def createSourceReaderActor(r: RadioSource): ActorRef = {
    val reader = createChildActor(RadioStreamActor(config, StreamReference(r.uri), self, eventManager))
