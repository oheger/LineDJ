...
/**
  ...
  * switches to another radio source. Note that there is only a change event
  * and not a pair of started/finished events. The end of a radio source is
  * typically less important. It can be determined implicitly when the next
  * change event arrives or a [[RadioPlaybackStoppedEvent]] event, in case that the
  * player was stopped.
  *
