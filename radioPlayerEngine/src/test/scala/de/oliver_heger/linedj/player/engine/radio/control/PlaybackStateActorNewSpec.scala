...
/**
  * Test class for [[PlaybackStateActorNew]].
  */
class PlaybackStateActorNewSpec extends AnyFlatSpec with BeforeAndAfterAll with Matchers
  ...
  "PlaybackStateActor" should "start playback if a source is available" in :
    ...
    helper.sendCommand(PlaybackStateActorNew.PlaybackSource(source))
      .expectNoPlaybackCommand()
      .sendCommand(PlaybackStateActorNew.StartPlayback)
  ...
  it should "start playback if a source becomes available later" in :
    ...
    helper.sendCommand(PlaybackStateActorNew.StartPlayback)
      .expectNoPlaybackCommand()
      .sendCommand(PlaybackStateActorNew.PlaybackSource(source))
      .expectPlaybackCommand(RadioStreamPlaybackActor.PlayRadioSource(source))
      .expectNoPlaybackCommand()
  ...
  it should "stop playback" in :
    ...
    helper.sendCommand(PlaybackStateActorNew.PlaybackSource(radioSource(8)))
      .sendCommand(PlaybackStateActorNew.StartPlayback)
      .skipPlaybackCommands(1)
      .sendCommand(PlaybackStateActorNew.StopPlayback)
  ...
  it should "switch to another source" in :
    ...
    helper.sendCommand(PlaybackStateActorNew.PlaybackSource(source1))
      .sendCommand(PlaybackStateActorNew.StartPlayback)
      .skipPlaybackCommands(1)
      .sendCommand(PlaybackStateActorNew.PlaybackSource(source2))
  ...
  it should "return the initial current playback state" in :
    val probeClient = testKit.createTestProbe[PlaybackStateActorNew.CurrentPlaybackState]()
    val helper = new PlaybackActorTestHelper

    helper.sendCommand(PlaybackStateActorNew.GetPlaybackState(probeClient.ref))

    probeClient.expectMessage(PlaybackStateActorNew.CurrentPlaybackState(None, None, playbackActive = false))
  ...
  it should "return the current playback state if a source is played" in :
    ...
    helper.sendCommand(PlaybackStateActorNew.PlaybackSource(source))
      .sendCommand(PlaybackStateActorNew.StartPlayback)
      .sendCommand(PlaybackStateActorNew.SourceSelected(source))
      .sendCommand(PlaybackStateActorNew.GetPlaybackState(probeClient.ref))

    probeClient.expectMessage(PlaybackStateActorNew.CurrentPlaybackState(Some(source), Some(source),
      playbackActive = true))
  ...
  it should "return the current playback state if a source is available, but playback is disabled" in :
    ...
    helper.sendCommand(PlaybackStateActorNew.SourceSelected(source))
      .sendCommand(PlaybackStateActorNew.PlaybackSource(source))
      .sendCommand(PlaybackStateActorNew.GetPlaybackState(probeClient.ref))
  ...
  private class PlaybackActorTestHelper:
    ...
    private val playbackActor =
      testKit.spawn(PlaybackStateActorNew.behavior(probePlaybackActor.ref))
    ...
    /**
      ...
      */
    def sendCommand(command: PlaybackStateActorNew.PlaybackStateCommand): PlaybackActorTestHelper =
